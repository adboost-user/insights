...
{
    public partial class LineItemDetail : System.Web.UI.Page
    {
        DfpUser user;
        DfpApi vGapi;
        ReportDfp vReport;
        ...
        public void getChartData(long vOrderId, string opc)
        {
            ...
            try
            {
                ...
                LitTitulo.Text = "";
                LitGrafInfo.Text = "";
                dataTable = vReport.ReportbyLine(vOrderId, opc);
                if (dataTable.Rows.Count > 0)
                {
                    ...
                    sb.Append("]");
                    sbData2.Append("]");
                    LitTitulo.Text = " Anuncio " + Convert.ToString(dataTable.Rows[0][0]);
        ...
        protected void LineaTotales(string vLineID)
        {
            string vTotImpresiones = "";
            string vTotClics = "";
            string vCTR = "";
            try
            {
                ...
                Decimal vDecTot = 0;
                Decimal vDecClic = 0;
                DataTable vTotalesDT = vGapi.GetLineTotFiltByLineId(vLineID);
                ...
                if (vTotalesDT.Rows.Count != 0)
                {
                    ...
                    LitTitle.Text = " Anuncio " + Convert.ToString(vTotalesDT.Rows[0][0]) + " Status " + Convert.ToString(vTotalesDT.Rows[0][6]);
                    //vOrder = Convert.ToInt64(vTotalesDT.Rows[0][5]);
                    vTotImpresiones = vDecTot.ToString("0,0");
                    vTotClics = vDecClic.ToString("0,0");
                    vCTR = vDecimal.ToString("P");
