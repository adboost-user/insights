...
namespace AdBoostDashboard
{
    public partial class OrderItem : System.Web.UI.Page
    {
        ...
        protected void OrdenTotales(string vCompanyId, long vOrderID)
        {
            string vTotImpresiones = "";
            string vTotClics = "";
            string vCTR = "";
            try
            {
                ...
                Decimal vDecTot = 0;
                Decimal vDecClic = 0;
                DataTable vTotalesDT = vGapi.GetOrderTotalsByOrderID(vCompanyId, vOrderID);
                string vTitulo = "";
                //vOrder = 0;
                if (vTotalesDT.Rows.Count != 0)
                {
                    ...
                    LitTitle.Text =" Campaña " + Convert.ToString(vTotalesDT.Rows[0][0]);
                    //vOrder = Convert.ToInt64(vTotalesDT.Rows[0][5]);
                    vTotImpresiones = vDecTot.ToString("0,0");
                    vTotClics = vDecClic.ToString("0,0");
                    vCTR = vDecimal.ToString("P");
